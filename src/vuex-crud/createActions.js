const createActions = ({ actions, rootUrl, client, only, parseList, parseSingle, parseError }) => {
    ...
    if (only.includes(FETCH_LIST)) {
        Object.assign(crudActions, {
            ...
            fetchList({ commit }, { config, customUrl, customUrlFnArgs = [] } = {}) {
                ...
                return client
                    .get(urlGetter({ customUrl, customUrlFnArgs, type: FETCH_LIST }), config)
                    .then((res) => {
    ...
    if (only.includes(FETCH_SINGLE)) {
        Object.assign(crudActions, {
             ...
             */
            fetchSingle({ commit }, { id, config, customUrl, customUrlFnArgs = [] } = {}) {
    ...
    if (only.includes(CREATE)) {
        Object.assign(crudActions, {
            ...
            create({ commit }, { data, config, customUrl, customUrlFnArgs = [] } = {}) {
                ...
                return client
                    .post(urlGetter({ customUrl, customUrlFnArgs, type: CREATE }), data, config)
                    .then((res) => {
                        const parsedResponse = parseSingle(res);
    ...
    if (only.includes(UPDATE)) {
        Object.assign(crudActions, {
             ...
             */
            update({ commit }, { id, data, config, customUrl, customUrlFnArgs = [] } = {}) {
                commit('updateStart');
    ...
    if (only.includes(REPLACE)) {
        Object.assign(crudActions, {
             ...
             * Update a single resource.
             */
            replace({ commit }, { id, data, config, customUrl, customUrlFnArgs = [] } = {}) {
                commit('replaceStart');
